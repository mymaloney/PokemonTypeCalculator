...
def recommend_types(coverage):
    recommendations = {}
    for defender in coverage.keys():
        recommended = []
        for atk_type in types:
            if any(type_chart[atk_type][defn] == 2 for defn in defender):
                recommended.append(atk_type)
        recommendations[defender] = recommended
...
# Main function to execute the coverage check and recommendations
def main():
    ...
    if not coverage:
        print("All defenders are covered by the current attackers.")
    else:
        print("Defenders not fully covered:")
        for defender, eff in coverage.items():
            print(f"{defender}: {eff}")
        
        recommendations = recommend_types(coverage)
        print("\nRecommended types to add to your team:")
        for defender, recs in recommendations.items():
            print(f"To counter {defender}, consider adding: {', '.join(recs) if recs else 'No strong recommendations'}")
